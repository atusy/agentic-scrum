...
* A PBI becomes `ready` when both roles `AGREE` it meets the Definition of Ready; if still `OBJECT` after 3 rounds, the facilitator decides against the Definition of Ready (default: stay `refining`) and records the dissent in the PBI's `notes`
